...
class Initor_for_btn(QtWidgets.QMainWindow):
    ...
    def init_right(self):
        ...
        self.show_video.setAlignment(Qt.AlignCenter)
        self.show_video.setText("等待识别结果")
        self.show_video.setFixedSize(
            self.video_size[0], self.video_size[1])  # width height
        # self.show_video.move(290, 20)
        self.show_video.setStyleSheet("QLabel{background:white;}"
                                     "QLabel{color:rgb(100,100,100);"
                                     "font-size:15px;"
                                     "font-weight:bold;font-family:宋体;}"
                                     "border-radius: 25px;border: 1px solid black;")
        ...
        self.right_bar_layout.addWidget(self.show_video, 1, 0)
    ...
    def init_left(self):
        ...
        self.left_button_3 = QtWidgets.QPushButton(
            qtawesome.icon('fa.image', color='white'), "本地图像文件")
...
class Initor_for_event(Initor_for_btn):
    ...
    def init_btn_event(self):
        ...
        self.left_button_3.clicked.connect(
            self.load_local_image_file)  # 点击选择文件
